...
/* Use  WHERE condition to justify those with full scores. Also use GROUP BY to group everything in the SELECT 
and use HAVING for count(h.hacker_id) > 1. Notice that the hacker_id in challenges is the the hacker who created the challenge
not the hacker who participated */
